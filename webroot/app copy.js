let userKey = '';
let filesToUpload = []; // Array di oggetti {file, relativePath}
...
const filesListDiv = document.getElementById('selected-files-list') || dropArea; // fallback

// [+] seleziona solo FILE multipli (no directory)
...
dropArea.addEventListener('drop', async (e) => {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (entry) {
            pending++;
            traverseFileTree(entry, "", function (file, relPath) {
                filesToUpload.push({file, relativePath: relPath});
            }, finishIfAllDone);
        } else if (items[i].getAsFile) {
            filesToUpload.push({file: items[i].getAsFile(), relativePath: items[i].getAsFile().name});
        }
    }
    if (!pending) updateSelectedFilesList();
});

// Ricorsione su cartelle (drag&drop)
function traverseFileTree(item, path, onFile, onDone) {
    path = path || '';
    if (item.isFile) {
        ...
        dirReader.readEntries(function (entries) {
            ...
            entries.forEach(function (entry) {
                traverseFileTree(entry, path + item.name + "/", onFile, function () {
                    left--;
...
// Gestione selezione file da <input type="file">
...
// Mostra la lista dei file scelti prima di upload
...
function updateSelectedFilesList() {
    // Trova o crea area
    let area = document.getElementById('selected-files-list');
    if (!area) {
    ...
    if (filesToUpload.length) {
        ...
        filesToUpload.forEach((obj) => {
            html += `<li title="${obj.relativePath}">${escapeHtml(obj.relativePath)} <span style="color:#999">(${formatSize(obj.file.size)})</span></li>`;
        });
        html += "</ul>";
        area.innerHTML = html;
...
// --- FILE UPLOAD MULTIPLO (batch: file, cartelle) ---
document.getElementById('upload-form').addEventListener('submit', async function (e) {
    e.preventDefault();
    if (!filesToUpload.length) return;
    let descInput = document.getElementById('descrizione-input');
    ...
    for (let obj of filesToUpload) {
        // Check max size lato client (se definito)
        ...
        formData.append('relativePath', obj.relativePath); // fondamentale!
        formData.append('descrizione', descInput.value || '');
...
// --- FILE LIST RENDERING (con cartelle) ---
function renderTree(tree, depth = 0) {
    ...
    for (const item of tree) {
        if (item.type === 'dir') {
            html += `<li>
                <span class="folder-label">${escapeHtml(item.name)}</span>
                ${renderTree(item.children, depth + 1)}
            </li>`;
        } else if (item.type === 'file') {
            // Se siamo nel livello root, niente rientro
...
// --- Tooltip per nome file lungo (opzionale, puoi tenerlo qui) ---

document.addEventListener('mouseover', function (e) {
  const el = e.target.closest('.file-name');
  if (el) {
    showCustomTooltip(el, el.dataset.fullname, e);
  }
});
